...
class Order
{
   ...
   private function add_order()
   {
      ...
      $this->add_order_items($this->pdo->lastInsertId());
      return $stmt->fetchAll(PDO::FETCH_ASSOC);
   }

   private function change_product(){
      $update_items=[];
      foreach ($this->items as $item) {
         $id=$item["id"];
         if (!isset($update_items[$id])){
            $update_items[$id]=1;
         } else {
            $update_items[$id]+=1;
         }
   ...
   private function add_order_items($order_id)
   {
      $update_items=$this->change_product();
      $stmt = $this->pdo->prepare("INSERT INTO `orders_items`( `order_id`, `product_id`, `count`) VALUES (?,?,?)");
      foreach ($update_items as $id=>$count) {
         $stmt->execute([$order_id, $id, $count]);
      }
      return "ok";
   }
   ...
   public function response()
   {
      ...
      return json_encode(["success" => 1, "data" => $order]);
   }
